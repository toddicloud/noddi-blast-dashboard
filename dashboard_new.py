...
# ============================
BASE_DIR = os.path.expanduser("~/Dropbox/mdt_noddi/main_directive_results")
...
def parse_question_simple(question, region_cols):
    ...
    # ----------------------------
    # REGION EXTRACTION — STRONG + FALLBACK
    # ----------------------------
    region_candidates = []

    # ✅ STRONG RULE: word AFTER the literal word "region"
    region_match = re.search(r"region\s+([a-zA-Z_]+)", q)
    explicit_key = region_match.group(1).lower() if region_match else None

    # ✅ FALLBACK: all long tokens
    tokens = re.findall(r"[a-zA-Z_]+", q)
    tokens = [t.lower() for t in tokens if len(t) >= 4]

    search_keys = []
    if explicit_key:
        search_keys.append(explicit_key)
    search_keys.extend(tokens)
    ...
    for key in search_keys:
        for col in region_cols:
            col_low = col.lower()
            roi_low = roi_names.get(col, "").lower()

            if key in col_low or key in roi_low:
                if col not in region_candidates:
...
if question:
    ...
    if task == "age_corrected_correlation" and region is not None:
